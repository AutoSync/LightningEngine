...
LightningEngine::Spectator::Spectator(Camera* Camera)
{
	Input = new Inputs(glfwGetCurrentContext());
	camera = Camera;
}

Camera* LightningEngine::Spectator::getCamera()
{
	return camera;
}

void LightningEngine::Spectator::Update()
{
...
void LightningEngine::Spectator::SetPespective(bool Perspective)
{
	camera->usePerspective = Perspective;
}

void LightningEngine::Spectator::AddInputMovement()
{
